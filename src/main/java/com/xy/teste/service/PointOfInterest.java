...
import com.xy.teste.dto.ApiDTOBuilder;
...
public class PointOfInterest implements IPointOfInterest {
	...
	@Override
	public List<PlaceDto> getPointsOfInterest(PositionDto positionDto) {
		Collection<Place> places = placeDAO.getPlaces();
		
		ArrayList<PlaceDto> pointsOfInterest = new ArrayList<PlaceDto>();
		for (Place place : places) {
			...
			{
				pointsOfInterest.add(ObjectMapperUtils.map(place, PlaceDto.class));
			}
		}
